...
fn get_hex(name: &str) -> String
{
    ...
    {
        let mut name_hex = line.split(';');
        if name == name_hex.next().unwrap()
        {
            return String::from(name_hex.next().unwrap());
        }
    }
    String::new()
}
...
fn get_per(hex: &str) -> String
{
    ...
    let g = u8::from_str_radix(&hex[3..5], 16).unwrap();
    let b = u8::from_str_radix(&hex[5..7], 16).unwrap();
    let s = format!("RGB({}%, {}%, {}%)", to_per(r), to_per(g), to_per(b));
    s
}

fn to_per(c: u8) -> u8
{
    (c as u16 * 100 as u16 / 255 as u16) as u8
}
